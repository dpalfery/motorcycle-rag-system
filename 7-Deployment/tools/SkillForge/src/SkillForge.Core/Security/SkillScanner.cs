...
namespace SkillForge.Core.Security;
...
public sealed class SkillScanner
{
    ...
    {
        ("SF-SEC-001", new(@"ignore\s+(all\s+)?(previous|prior|above)\s+instructions", RegexOptions.IgnoreCase | RegexOptions.Compiled), "prompt-injection: 'ignore previous instructions'"),
        ("SF-SEC-002", new(@"disregard\s+(your|all|the)\s+(guidelines|rules|instructions|policy|policies)", RegexOptions.IgnoreCase | RegexOptions.Compiled), "prompt-injection: 'disregard your guidelines'"),
        ("SF-SEC-003", new(@"\b(system\s+override|override\s+(the\s+)?system\s+prompt|reveal\s+(your\s+)?system\s+prompt)\b", RegexOptions.IgnoreCase | RegexOptions.Compiled), "prompt-injection: system-prompt override/exfiltration"),
        ("SF-SEC-004", new(@"you\s+are\s+now\s+(a|an|the)\b", RegexOptions.IgnoreCase | RegexOptions.Compiled), "persona hijack: 'you are now …'"),
        ("SF-SEC-005", new(@"\b(exfiltrat|send\s+(the\s+)?(api\s+key|secret|token|credential|password)s?\s+to|forward\s+.*\s+to\s+https?://)\b", RegexOptions.IgnoreCase | RegexOptions.Compiled), "data exfiltration verb"),
    };
    ...
    {
        ("SF-SEC-010", new(@"curl\s+[^\n|]*\|\s*(ba)?sh", RegexOptions.IgnoreCase | RegexOptions.Compiled), "remote code execution: 'curl … | sh'"),
        ("SF-SEC-011", new(@"wget\s+[^\n|]*\|\s*(ba)?sh", RegexOptions.IgnoreCase | RegexOptions.Compiled), "remote code execution: 'wget … | sh'"),
        ("SF-SEC-012", new(@"eval\s*\(?\s*\$?\(?\s*.*base64\s+(--?d|--decode)", RegexOptions.IgnoreCase | RegexOptions.Compiled), "obfuscated execution: eval of base64-decoded content"),
        ("SF-SEC-013", new(@"\b(rm\s+-rf\s+[~/]|:\(\)\s*\{\s*:\|:&\s*\};:)", RegexOptions.Compiled), "destructive command"),
    };

    private static readonly (string Code, Regex Rx, string Label)[] SecretPatterns =
    {
        ("SF-SEC-020", new(@"AKIA[0-9A-Z]{16}", RegexOptions.Compiled), "hardcoded AWS access key id"),
        ("SF-SEC-021", new(@"gh[pousr]_[A-Za-z0-9]{36,}", RegexOptions.Compiled), "hardcoded GitHub token"),
        ("SF-SEC-022", new(@"-----BEGIN\s+(RSA|EC|OPENSSH|PGP|DSA)?\s*PRIVATE KEY-----", RegexOptions.Compiled), "embedded private key"),
        ("SF-SEC-023", new(@"xox[baprs]-[A-Za-z0-9-]{10,}", RegexOptions.Compiled), "hardcoded Slack token"),
    };
    ...
    public IEnumerable<Diagnostic> Scan(Skill skill)
    {
        ...
        foreach (Match m in HtmlComment.Matches(body))
        {
            ...
            if (Regex.IsMatch(inner, @"\b(ignore|disregard|system|instruction|tool|exfiltrat|send|password|token|secret)\b", RegexOptions.IgnoreCase))
                yield return new Diagnostic("SF-SEC-006", Severity.Critical,
        ...
        {
            yield return new Diagnostic("SF-SEC-007", Severity.Warning,
        ...
        if (meta is null || !meta.ContainsKey("author"))
            yield return new Diagnostic("SF-SEC-030", Severity.Info,
        ...
        if (meta is null || !meta.ContainsKey("version"))
            yield return new Diagnostic("SF-SEC-031", Severity.Info,
        ...
        if (string.IsNullOrWhiteSpace(skill.Frontmatter.License))
            yield return new Diagnostic("SF-SEC-032", Severity.Info,
