...
public sealed class UsersAdminController : ControllerBase {
    ...
    public async Task<IActionResult> SetUserEnabledStatusAsync(
        ...
        try {
            ...
            _logger.LogInformation("Admin set user {UserId} enabled status to {IsEnabled}", LogSanitizer.Sanitize(userId), request.IsEnabled);  // codeql[cs/log-forging]
            return Ok(user);
        }
        catch (ArgumentException ex) {
            _logger.LogWarning(ex, "Invalid request to set user enabled status for {UserId}", LogSanitizer.Sanitize(userId));  // codeql[cs/log-forging]
            return BadRequest(new { error = ex.Message });
        }
        catch (Exception ex) {
            _logger.LogError(ex, "Error setting user enabled status for {UserId}", LogSanitizer.Sanitize(userId));  // codeql[cs/log-forging]
    ...
    public async Task<IActionResult> AssignPlanToUserAsync(
        ...
        try {
            ...
            _logger.LogInformation("Admin assigned plan {PlanId} to user {UserId}", LogSanitizer.Sanitize(request.PlanId), LogSanitizer.Sanitize(userId));  // codeql[cs/log-forging]
            return Ok(user);
        }
        catch (ArgumentException ex) {
            _logger.LogWarning(ex, "Invalid request to assign plan to user {UserId}", LogSanitizer.Sanitize(userId));  // codeql[cs/log-forging]
            return BadRequest(new { error = ex.Message });
        }
        catch (Exception ex) {
            _logger.LogError(ex, "Error assigning plan to user {UserId}", LogSanitizer.Sanitize(userId));  // codeql[cs/log-forging]
            return StatusCode(StatusCodes.Status500InternalServerError, new { error = "An error occurred" });
        }
    }
    ...
    public async Task<IActionResult> ChangeManagedUserTierAsync(
        ...
        try {
            ...
            _logger.LogWarning(ex, "Invalid tier-change request for user {UserId}", LogSanitizer.Sanitize(userId));  // codeql[cs/log-forging]
            return BadRequest(new { error = ex.Message });
        }
        catch (InvalidOperationException ex) {
            _logger.LogWarning(ex, "Tier change could not be completed for user {UserId}", LogSanitizer.Sanitize(userId));  // codeql[cs/log-forging]
    ...
    public async Task<IActionResult> CancelManagedUserAsync(
        ...
        try {
            ...
            _logger.LogWarning(ex, "Invalid cancellation request for user {UserId}", LogSanitizer.Sanitize(userId));  // codeql[cs/log-forging]
            return BadRequest(new { error = ex.Message });
        }
        catch (InvalidOperationException ex) {
            _logger.LogWarning(ex, "Cancellation could not be completed for user {UserId}", LogSanitizer.Sanitize(userId));  // codeql[cs/log-forging]
            return Conflict(new { error = ex.Message });
        }
    }
