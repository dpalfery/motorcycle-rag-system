...
#pragma warning restore CA3003 // Potential file path injection vulnerability
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Error processing file {FileName}", LogSanitizer.Sanitize(Path.GetFileName(request.FilePath)));  // codeql[cs/log-forging]
    ...
    public async Task<IActionResult> GetPipelineStatusAsync(string executionId)
    {
        ...
        {
            _logger.LogError(ex, "Error getting pipeline status for {ExecutionId}", LogSanitizer.Sanitize(executionId));  // codeql[cs/log-forging]
            return StatusCode(500, new ProblemDetails
            {
                Title = "Internal server error",
    ...
    public async Task<IActionResult> CancelPipelineAsync(string executionId)
    {
        ...
        {
            _logger.LogError(ex, "Error cancelling pipeline {ExecutionId}", LogSanitizer.Sanitize(executionId));  // codeql[cs/log-forging]
            return StatusCode(500, new ProblemDetails
            {
                Title = "Internal server error",
