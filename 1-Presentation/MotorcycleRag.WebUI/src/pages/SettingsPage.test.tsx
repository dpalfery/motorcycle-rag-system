...
function mockResponse(partial: Pick<Response, "ok" | "json">): Response {
...
describe("SettingsPage", () => {
  ...
  it("records an error for unsupported file types", async () => {
    render(<SettingsPage />);
    dropFile(getDropZone(), makeFile("notes.txt"));
  ...
  it("uploads a PDF and starts processing with OCR config", async () => {
    vi.mocked(fetch)
      ...
      .mockResolvedValueOnce(mockResponse({ ok: true, json: async () => ({}) }));

    render(<SettingsPage />);
    dropFile(getDropZone(), makeFile("manual.pdf"));
  ...
  it("uploads a CSV without start configuration", async () => {
    ...
    render(<SettingsPage />);
    dropFile(getDropZone(), makeFile("specs.csv"));
  ...
  it("surfaces start-job failures using extension ids and non-json bodies", async () => {
    ...
    render(<SettingsPage />);
    dropFile(getDropZone(), makeFile("data.csv"));
