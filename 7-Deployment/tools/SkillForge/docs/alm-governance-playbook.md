...
- **It can be malicious.** A skill shapes agent behavior and can bundle scripts, so an untrusted or AI-generated skill is a trust surface. Review it the way you'd review a third-party dependency. SkillForge's `scan` is the automated first pass; it does not replace human review.
...
- A skill is uploaded as a single `SKILL.md` or a `.zip` bundling `SKILL.md` + `scripts/`/`references/`/`assets/`. `skillforge pack` produces that `.zip`.
...
*This playbook pairs with the SkillForge CLI. Everything here is enforceable: each section maps to a command you can put in CI today.*
