...
namespace SkillForge.Core.Docs.CodeGraph;

/// <summary>One resolved code symbol from the CodeGraph index.</summary>
public sealed record CodeGraphNode(string Id, string Kind, string Name, string QualifiedName, string FilePath);
...
public sealed class CodeGraphResolver
{
    ...
    private void Load()
    {
        ...
        foreach (var line in RunSqlite(sql))
        {
            ...
            var node = new CodeGraphNode(parts[0], parts[1], parts[2], parts[3], parts[4]);
