...
public static class CommandHelpers
{
    ...
    public static SkillSet? LoadOrReport(string path, DiagnosticReport report)
    {
        ...
        foreach (var r in results)
        {
            ...
            else report.Add(new Diagnostic("SF-PARSE-000", Severity.Error,
    ...
    public static void Finish(DiagnosticReport report, AnalysisSettings settings, string command)
    {
        if (settings.NoInfo)
        {
        ...
        ReportRenderer.Render(report, settings.ParsedFormat, command);
