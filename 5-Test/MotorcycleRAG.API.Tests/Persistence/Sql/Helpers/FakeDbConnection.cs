...
internal sealed class TestableSqlConnectionFactory : ISqlConnectionFactory
{
    ...
    {
        var command = connection.CreateCommand();
        command.CommandText = commandText; // nosemgrep: csharp.lang.security.sqli.csharp-sqli.csharp-sqli — test-only FakeDbConnection; commandText is test-controlled literal SQL
        return command;
    }
