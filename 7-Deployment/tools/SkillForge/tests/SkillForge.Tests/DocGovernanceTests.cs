...
using SkillForge.Core.Docs.Export;
using SkillForge.Core.Docs.Model;
using SkillForge.Core.Docs.Parsing;
using SkillForge.Core.Docs.Validation;
using Xunit;
...
public class DocDriftLinterTests
{
    ...
    public void Missing_Index_Is_Reported_As_Critical_Not_Silently_Passed()
    {
        ...
        var resolver = new Core.Docs.CodeGraph.CodeGraphResolver(fixture.Root);
...
{
    [Fact]
    public void SkillName_Alias_Still_Reads_Subject()
    {
        var diagnostic = new Diagnostic("SF-DOC-SPEC-001", Severity.Error, "message", "subject-value");

        Assert.Equal("subject-value", diagnostic.Subject);
        Assert.Equal("subject-value", diagnostic.SkillName);
    }
}
