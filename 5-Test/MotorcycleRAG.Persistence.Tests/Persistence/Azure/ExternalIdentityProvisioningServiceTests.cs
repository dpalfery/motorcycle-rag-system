...
public sealed class ExternalIdentityProvisioningServiceTests
{
    ...
    [Fact]
    public async Task ProvisionApprovedUserAsync_ShouldThrowInvalidOperationException_WhenInviteRedirectUrlIsMissing()
    {
        // User not found, but invite can't be sent because redirect URL is missing
        var handler = CreateOrderedHandler(
            Plan(HttpMethod.Get, UsersLookupUrl("rider@example.com"), () => JsonResponse(HttpStatusCode.OK, """{ "value": [] }""")));

        using var httpClient = handler.CreateClient();
        var sut = CreateSut(httpClient, options => options.InviteRedirectUrl = string.Empty);

        var act = async () => await sut.ProvisionApprovedUserAsync(
            "rider@example.com", "Rider", TierLabel.Trial, IdentityProvider.Microsoft);

        await act.Should().ThrowAsync<InvalidOperationException>()
            .WithMessage("*InviteRedirectUrl*");
    }

    [Fact]
    public async Task ProvisionApprovedUserAsync_ShouldThrowInvalidOperationException_WhenInviteRedirectUrlIsNotAbsoluteUri()
    {
        // User not found, invite with non-absolute URI fails
        var handler = CreateOrderedHandler(
            Plan(HttpMethod.Get, UsersLookupUrl("rider@example.com"), () => JsonResponse(HttpStatusCode.OK, """{ "value": [] }""")));

        using var httpClient = handler.CreateClient();
        var sut = CreateSut(httpClient, options => options.InviteRedirectUrl = "not-a-valid-uri");

        var act = async () => await sut.ProvisionApprovedUserAsync(
            "rider@example.com", "Rider", TierLabel.Trial, IdentityProvider.Microsoft);

        await act.Should().ThrowAsync<InvalidOperationException>()
            .WithMessage("*InviteRedirectUrl must be an absolute URI*");
    }

    [Fact]
    public async Task ProvisionApprovedUserAsync_ShouldUseEmailAsDisplayName_WhenDisplayNameIsEmpty()
    {
        var externalDirectoryObjectId = "f0000000-0000-0000-0000-000000000000";
        ...
        var handler = CreateOrderedHandler(
            ...
            Plan(
                ...
                request => AssertInvitationPayload(
                    request,
                    "rider@example.com",
                    "rider@example.com",  // display name falls back to email
                    InviteRedirectUrl,
                    true)),
            Plan(HttpMethod.Get, ServicePrincipalUrl(), () => JsonResponse(HttpStatusCode.OK, ServicePrincipalResponseJson())),
            Plan(HttpMethod.Get, UserAssignmentsUrl(externalDirectoryObjectId), () => JsonResponse(HttpStatusCode.OK, """{ "value": [] }""")),
            Plan(HttpMethod.Post, CreateAssignmentUrl(), () => JsonResponse(HttpStatusCode.Created, """{ "id": "assignment-trial" }"""),
                request => AssertAssignmentPayload(request, externalDirectoryObjectId, ApiServicePrincipalObjectId, TrialAppRoleId)));

        using var httpClient = handler.CreateClient();
        var sut = CreateSut(httpClient);

        var result = await sut.ProvisionApprovedUserAsync(
            "rider@example.com", "  ", TierLabel.Trial, IdentityProvider.Microsoft);

        result.Should().Be(externalDirectoryObjectId);
    }

    [Fact]
    public async Task RevokeAccessAsync_ShouldSucceed_WhenNoAssignmentsExist()
    {
        var externalDirectoryObjectId = "111111aa-aaaa-aaaa-aaaa-aaaaaaaaaaaa";
        var handler = CreateOrderedHandler(
            Plan(HttpMethod.Get, ServicePrincipalUrl(), () => JsonResponse(HttpStatusCode.OK, ServicePrincipalResponseJson())),
            Plan(HttpMethod.Get, UserAssignmentsUrl(externalDirectoryObjectId), () => JsonResponse(HttpStatusCode.OK, """{ "value": [] }""")));
        ...
        handler.VerifyRequest(HttpMethod.Get, ServicePrincipalUrl(), Times.Once());
        handler.VerifyRequest(HttpMethod.Get, UserAssignmentsUrl(externalDirectoryObjectId), Times.Once());
    }

    [Fact]
    public async Task ProvisionApprovedUserAsync_ShouldHandleInvitationWithFallbackLookup_WhenInvitedUserIdIsMissing()
    {
        var externalDirectoryObjectId = "222222bb-bbbb-bbbb-bbbb-bbbbbbbbbbbb";
        // Invitation response has no invitedUser.id, but fallback lookup finds the user
        var handler = CreateOrderedHandler(
            Plan(HttpMethod.Get, UsersLookupUrl("rider@example.com"), () => JsonResponse(HttpStatusCode.OK, """{ "value": [] }""")),
            Plan(HttpMethod.Post, $"{GraphBaseUrl}/invitations", () => JsonResponse(HttpStatusCode.Created, """{ "status": "Accepted" }""")),
            Plan(HttpMethod.Get, UsersLookupUrl("rider@example.com"), () => JsonResponse(HttpStatusCode.OK, $$"""
    ...
    [Fact]
    public async Task ProvisionApprovedUserAsync_ShouldSendCustomInvitationMessage_WhenConfigured()
    {
        var externalDirectoryObjectId = "333333cc-cccc-cccc-cccc-cccccccccccc";
        var handler = CreateOrderedHandler(
            Plan(HttpMethod.Get, UsersLookupUrl("rider@example.com"), () => JsonResponse(HttpStatusCode.OK, """{ "value": [] }""")),
            Plan(
                HttpMethod.Post,
                ...
                async request =>
                {
                    var payload = await request.Content!.ReadFromJsonAsync<Dictionary<string, object?>>();
                    payload.Should().NotBeNull();
                    payload!["invitedUserMessageInfo"].Should().NotBeNull();
                }),
            Plan(HttpMethod.Get, ServicePrincipalUrl(), () => JsonResponse(HttpStatusCode.OK, ServicePrincipalResponseJson())),
            Plan(HttpMethod.Get, UserAssignmentsUrl(externalDirectoryObjectId), () => JsonResponse(HttpStatusCode.OK, AssignmentsResponseJson(
                AssignmentJson("assignment-trial", TrialAppRoleId, ApiServicePrincipalObjectId)))));
        ...
        using var httpClient = handler.CreateClient();
        var sut = CreateSut(httpClient, options =>
        {
            options.InvitationMessageBody = "Welcome to MotorcycleRAG!";
            options.InvitationMessageLanguage = "en-US";
        });
    ...
    private static Mock<HttpMessageHandler> CreateOrderedHandler(params ResponsePlan[] plans)
    {
        ...
        handler
            ...
            .Returns( async (HttpRequestMessage request, CancellationToken _) =>
            {
                ...
                if (plan.AssertRequest is not null)
                    await plan.AssertRequest(request);
    ...
    private static ResponsePlan Plan(
        ...
        Func<HttpRequestMessage, Task>? assertRequest = null) =>
    ...
    private static async Task AssertInvitationPayload(
    ...
    private static async Task AssertAssignmentPayload(
        HttpRequestMessage request,
    ...
    // ---- ResolveTargetRoleId with invalid GUID ----

    [Fact]
    public async Task ReconcileTierAssignmentsAsync_ShouldThrowInvalidOperationException_WhenConfiguredRoleIdIsNotValidGuid()
    {
        var externalDirectoryObjectId = "99999999-9999-9999-9999-999999999999";
        var handler = CreateOrderedHandler(
            Plan(HttpMethod.Get, ServicePrincipalUrl(), () => JsonResponse(HttpStatusCode.OK, ServicePrincipalResponseJson())));
        ...
        var sut = CreateSut(httpClient, options =>
        {
            options.TrialAppRoleId = "not-a-valid-guid";
        });
    ...
    // ---- CreateManagedAssignmentAsync with non-400/409 error ----

    [Fact]
    public async Task ReconcileTierAssignmentsAsync_ShouldPropagateError_WhenAssignmentCreationFailsWithServerError()
    {
        var externalDirectoryObjectId = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaab";
        var handler = CreateOrderedHandler(
            Plan(HttpMethod.Get, ServicePrincipalUrl(), () => JsonResponse(HttpStatusCode.OK, ServicePrincipalResponseJson())),
            Plan(HttpMethod.Get, UserAssignmentsUrl(externalDirectoryObjectId), () => JsonResponse(HttpStatusCode.OK, """{ "value": [] }""")),
            Plan(HttpMethod.Post, CreateAssignmentUrl(), () => JsonResponse(HttpStatusCode.InternalServerError, """
                {
                  "error": {
        ...
        await act.Should().ThrowAsync<InvalidOperationException>()
            .WithMessage("*Assign API app role failed*");
    }

    // ---- ListManagedAssignmentsAsync when user not found (404) ----

    [Fact]
    public async Task ReconcileTierAssignmentsAsync_ShouldThrow_WhenUserNotFound()
    {
        var externalDirectoryObjectId = "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb";
        var handler = CreateOrderedHandler(
            Plan(HttpMethod.Get, ServicePrincipalUrl(), () => JsonResponse(HttpStatusCode.OK, ServicePrincipalResponseJson())),
            Plan(HttpMethod.Get, UserAssignmentsUrl(externalDirectoryObjectId), () => EmptyResponse(HttpStatusCode.NotFound)));
        ...
        await act.Should().ThrowAsync<InvalidOperationException>()
            .WithMessage("*not found in Microsoft Graph*");
    }
