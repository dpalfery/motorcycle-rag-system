...
- Shared DTOs are property bags and must not acquire domain invariants or persistence-row behavior. Test those behaviors at the owning Domain or Persistence boundary instead.
- Keep one behavior-focused test class per top-level DTO when custom behavior exists; pure property bags require no padding tests.
