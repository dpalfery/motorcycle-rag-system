...
namespace SkillForge.Cli.Rendering;
...
{
    public static void Render(DiagnosticReport report, OutputFormat format, string command)
    {
        switch (format)
        {
            ...
            case OutputFormat.Markdown: Console.WriteLine(ToMarkdown(report, command)); break;
    ...
    private static void RenderTable(DiagnosticReport report)
    {
        ...
        foreach (var d in report.Items.OrderByDescending(i => i.Severity))
        {
            ...
            table.AddRow(
                ...
                new Markup(Markup.Escape(d.SkillName)),
    ...
    private static string ToMarkdown(DiagnosticReport report, string command)
    {
        var sb = new StringBuilder();
        sb.AppendLine($"### SkillForge `{command}` results");
        sb.AppendLine();
        ...
        sb.AppendLine("| Severity | Code | Skill | Message |");
        sb.AppendLine("|---|---|---|---|");
        foreach (var d in report.Items.OrderByDescending(i => i.Severity))
            sb.AppendLine($"| {d.Severity} | {d.Code} | {d.SkillName} | {d.Message.Replace("|", "\\|")} |");
        return sb.ToString();
    }
            ...
            ["runs"] = new JsonArray
            {
                new JsonObject
                {
                    ["tool"] = new JsonObject
                    {
                        ...
                        {
                            ["name"] = "SkillForge",
                            ["informationUri"] = "https://github.com/bonaniibm/SkillForge",
                            ["version"] = "0.1.0",
                            ["rules"] = rules
                        }
                    },
