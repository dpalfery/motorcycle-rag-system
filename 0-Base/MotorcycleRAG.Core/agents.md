...
- Configuration rules: root `AGENTS.md`. C#/.NET code uses Azure App Configuration + Key Vault; environment variables are limited to the Python local processor runtime values set by the Admin app.
...
- Don’t introduce “config” helpers that read secrets from files or environment variables. If a value might be secret, design a small option/abstraction and let outer layers populate it from Azure App Configuration + Key Vault.
